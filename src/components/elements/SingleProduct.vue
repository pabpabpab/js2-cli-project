    ...
    <div class="singleProduct">
      <div class="container singleProduct_img_container">
        <a href="#" class="product_img_link"
           @click.prevent="nextPhoto(currentPhotoIndex)" v-html="productImg">
        </a>
      </div>
      <a href="#" class="left_flyer" @click.prevent="prevPhoto(currentPhotoIndex)">
        <i class="fas fa-angle-left"></i>
      </a>
      <a href="#" class="right_flyer" @click.prevent="nextPhoto(currentPhotoIndex)">
    ...
    <div class="container singleProduct_info">
      ...
      <div class="material_and_designer">
        <div class="material">MATERIAL:
          <p class="material_value">{{product.material}}</p>
        </div>
        <div class="designer">DESIGNER:
          <p class="designer_value">{{product.designer}}</p>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      product: {},
      // почему-то в этом компоненте не работает относительный путь до картинок
      ...
      inputSize: '',
      inputQuantity: 0,
      cartAPI: this.$parent.$parent.$refs.header.$refs.cart,
  ...
  methods: {
    ...
    addProduct(product) {
      ...
      if (message) {
        ...
        this.$parent.$parent.$refs.message.setMessage(message);
        return;
      }
      /**
       ...
       */
      const find = this.cartAPI.cartItems.find((el) => el.product_name === product.product_name);
      /* eslint max-len: ["error", { "code": 190 }] */
      const find2 = this.cartAPI.cartItems.find((el) => ((el.product_name === product.product_name) && (el.color === this.inputColor) && (el.size === this.inputSize.split(',')[0])));
      switch (true) {
        case (!find):
          // добавляем как абсолютно новый товар если: нет такого товара с таким названием в корзине
          this.addNewProduct(product, product.id_product);
          this.$parent.$parent.$refs.message.setMessage('Товар добавлен.');
          break;
        case (!find2):
          // если есть, но цвет или размер другой,
          // то тоже добавляем как абсолютно новый товар с новым id
          this.addNewProduct(product, this.getNewProductId());
          this.$parent.$parent.$refs.message.setMessage('Товар добавлен,<br>'
            + 'но добавлен как новый с новым id, '
            + 'потому что цвет или размер не совпадает с таким же товаром в корзине.');
          break;
        default:
          // если все параметры с найденным в корзине совпадают, то просто увеличить кол-во товара
          this.$parent.$parent.putJson(`/api/cart/${find2.id_product}`, { quantity: +this.inputQuantity })
            .then((data) => {
              if (data.result === 1) {
                find.quantity = data.newQuantity;
              }
            });
          this.$parent.$parent.$refs.message.setMessage('Количество товара увеличено.');
      }
    },

    // формируем новый товар с введенными пользователем параметрами и с новым id
    addNewProduct(product, newId) {
      const newProduct = { ...this.product, ...{ quantity: +this.inputQuantity } };
      newProduct.color = this.inputColor;
      [newProduct.size, newProduct.size_number] = this.inputSize.split(',');
      newProduct.size_number = +newProduct.size_number;
      newProduct.id_product = newId;
      this.$parent.$parent.postJson('/api/cart', newProduct)
        .then((data) => {
          if (data.result === 1) {
    ...
    makeProductImg() {
      ...
      // поэтому так (меняем целиком html)
      this.productImg = `<img src="${this.imgPath}${this.product.img[this.currentPhotoIndex]}" alt="product" class="product_img appearance">`;
    },
  },

  mounted() {
    // вытянуть один товар по его id
    // const url = `/api/products/${this.$route.params.id}`;
    // товар 678 в json файле на сервере лучше настроен для страницы SingleProduct
    const url = '/api/products/678';
    this.$parent.$parent.getJson(url)
      .then((data) => {
